...
import { exclideTags } from '@/utils/sanitize';
  ...
  <Link className="w-full" href={`${PATH.POST}/${post.id}`}>
    <Card className="relative h-full bg-white text-gray-800 hover:bg-gray-100 dark:bg-gray-900 dark:text-gray-400 dark:hover:bg-black">
      <div className="absolute right-0 top-0 p-3">
        <ActionIconButton className="px-4 py-2 text-base" label={post.published ? '공개' : '비공개'}>
          {post.published ? <Unlock className="size-5" /> : <Lock className="size-5" />}
        </ActionIconButton>
      </div>
      <CardHeader>
        <CardTitle className="line-clamp-1 text-base sm:text-xl">
      ...
      <CardContent className="h-12">
        <div className="line-clamp-2 text-sm sm:text-base">{exclideTags(post.content.slice(0, 300))}</div>
      </CardContent>
      <CardFooter>
